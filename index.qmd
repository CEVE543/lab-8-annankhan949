...
author: CEVE 543 Fall 2025 ak276
date: "2025-10-31"
type: "lab"
module: 2
week: 11
...
*It calculates the euclidean distance between a point and each centroid, then assigns the point to the nearest centroid*
- Explain in your own words what the `update_centroids` function does and why it's important for k-means clustering.
*it computes the mean location of each point in a cluster and then updates the centroid to be that location. This is important in kmeans becuase it can be used iteratively with reassigning centroids to better represent groups*
...
*With euclidean distance the updated center of the centroid is the mean of the points, with absolute it would be the median*
...
*Based on the anomolies each day matches its assigned cluster pattern well*
- Do your EOF spatial patterns match those shown in the paper?
*Similar but not exactly the same, higher resolution for the ones in the paper, EOF 1 accounts for slightly more variability*
...
*Not entirely, could be due to initialization of centroids, EOFs, or data precprocessing?*
...
*With no rescaling on the PCs PC1 tends to dominate over the others changing the WT's*
- What is the impact of changing the number of clusters (k) on the weather type patterns?
*More k's can lead to more specific grouping but can also over-group*